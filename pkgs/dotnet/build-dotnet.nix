...
installPhase = ''
  ...
  echo "#!/bin/sh" > $out/bin/dotnet-wrapper
  echo "export DOTNET_SYSTEM_GLOBALIZATION_INVARIANT=1" >> $out/bin/dotnet-wrapper
  echo "exec $out/dotnet \"\$@\"" >> $out/bin/dotnet-wrapper

  # Make the wrapper script executable
  chmod +x $out/bin/dotnet-wrapper

  # Link the wrapper script instead of the original binary
  ln -s $out/bin/dotnet-wrapper $out/bin/dotnet
